...
public final class FSUBVariablesInstrumenter implements OpCodeInstrumenter {
    @Override
    public void instrument(MethodVisitor mv, int opCode, String classPath, Methods methods, Reaction reaction, LocalVariablesSorter src) {
        ...
        reaction.insertReactionCall(mv, "Precision error : FSUB", methods, classPath);
        Label l1 = new Label();
        ...
        reaction.insertReactionCall(mv, "Result is Double.POSITIVE_INFINITY : FSUB", methods, classPath);
        mv.visitJumpInsn(GOTO, l1);
        mv.visitLabel(l2);
        src.visitVarInsn(FLOAD, r);
        mv.visitLdcInsn(new Float("-Infinity"));
        ...
        reaction.insertReactionCall(mv, "Result is Double.NEGATIVE_INFINITY : FSUB", methods, classPath);
        mv.visitLabel(l1);
        src.visitVarInsn(FLOAD, r);
    }
}