...
public final class DDIVVariablesInstrumenter implements OpCodeInstrumenter {
    @Override
    public void instrument(MethodVisitor mv, int opCode, String classPath, Methods methods, Reaction reaction, LocalVariablesSorter src) {
        ...
        reaction.insertReactionCall(mv, "Result is Double.POSITIVE_INFINITY : DDIV", methods, classPath);
        mv.visitJumpInsn(GOTO, l1);
        mv.visitLabel(l2);
        src.visitVarInsn(DLOAD, r);
        mv.visitLdcInsn(new Double("-Infinity"));
        ...
        reaction.insertReactionCall(mv, "Result is Double.NEGATIVE_INFINITY : DDIV", methods, classPath);
        mv.visitLabel(l1);
        src.visitVarInsn(DLOAD, r);
    }
}
