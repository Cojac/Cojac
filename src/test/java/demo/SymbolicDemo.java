...
public class SymbolicDemo{
    ...
    public static void runFx( int fx, double x ) throws InvocationTargetException, IllegalAccessException,
        ...
        x = COJAC_MAGIC_asSymbolicTarget( x );
        double res = ( double ) f.invoke( SymbolicDemo.class, x );
        double dfRes = ( double ) df.invoke( SymbolicDemo.class, x );
        double dRes = COJAC_MAGIC_getSymbolic( res );
        System.out.printf( "f%d(x) = %s\n", fx, res );
        System.out.printf( "f%d'(x) = %s should be %s\n", fx, COJAC_MAGIC_getSymbolic( res ), dfRes );
