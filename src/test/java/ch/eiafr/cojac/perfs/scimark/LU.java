...
/**
 * LU matrix factorization. (Based on TNT implementation.) Decomposes a matrix A  into a triangular lower triangular
 * factor (L) and an upper triangular factor (U) such that A = L*U.  By convnetion, the main diagonal of L consists of
 * 1's so that L and U can be stored compactly in a NxN matrix.
 */
public class LU {
    /**
     * Returns a <em>copy</em> of the compact LU factorization. (useful mainly for debugging.)
     *
     * @return the compact LU factorization.  The U factor is stored in the upper triangular portion, and the L factor
     *         is stored in the lower triangular portion. The main diagonal of L consists (by convention) of ones, and
     ...
     *
     * @return the pivot vector used in obtaining the LU factorzation.  Subsequent solutions must permute the right-hand
     *         side by this vector.
     */
    public int[] getPivot() {
     ...
     * @param pivot (in) the pivot vector which lists the reordering used during the factorization stage.
